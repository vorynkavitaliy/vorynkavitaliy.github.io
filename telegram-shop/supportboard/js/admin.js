                    ...
                    SBF.ajax({
                        ...
                        if (response['orders'].length) {
                            ...
                            for (var i = 0; i < response['orders'].length; i++) {
                                ...
                                code += `<div data-id="${id}"><span><span>#${id}</span> <span>${SBF.beautifyTime(order['date_created_gmt'], true)}</span><a href="${SITE_URL}/wp-admin/post.php?post=${id}&action=edit" target="_blank" class="sb-icon-next"></a></span><div></div></div>`;
                            }
                            code += '</div>';
                        }
            ...
            conversationPanelOrder: function (order_id) {
                ...
                SBF.ajax({
                    ...
                    if (response) {
                        ...
                        code += `<div class="sb-title">${sb_('Order total')}: <span>${response['currency_symbol']}${response['total_sales']}<span></div><div class="sb-title">${sb_('Order status')}: <span>${SBF.slugToString(response['status'].replace('wc-', ''))}<span></div><div class="sb-title">${sb_('Date')}: <span>${SBF.beautifyTime(response['date_created_gmt'], true)}<span></div><div class="sb-title">${sb_('Products')}</div>`;
    ...
    */
    ...
    var SBProfile = {
        ...
        profileRow: function (key, value, name = key) {
            if (value == '') return '';
            let icons = { 'id': 'padlock', 'full_name': 'user', 'email': 'envelope', 'phone': 'phone', 'user_type': 'user', 'last_activity': 'calendar', 'creation_time': 'calendar', 'token': 'shuffle', 'currency': 'currency', 'browser': 'smartphone', 'ip': 'internet', 'location': 'marker', 'country': 'marker', 'address': 'marker', 'city': 'marker', 'os': 'desktop', 'current_url': 'next', 'timezone': 'clock' };
