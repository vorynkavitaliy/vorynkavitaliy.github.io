...
(function ($) {
    ...
    class SBConversation {
        ...
        getCode() {
            ...
            if (count) {
                ...
                return `<div class="sb-conversation-item" data-user-id="${message.get('user_id')}"><img src="${message.get('profile_image')}"><div><span class="sb-name">${sb_(!admin && message.get('last_name').charAt(0) == '#' ? 'You' : message.get('full_name'))}</span><span class="sb-time">${SBF.beautifyTime(message.get('creation_time'))}</span></div><div class="sb-message">${text.length > 114 ? text.substr(0, 114) + ' ...' : text}</div></div>`;
            }
            return '';
        }
    ...
    */
    ...
    var SBChat = {
        ...
        sendMessage: function (user_id = -1, message = '', attachments = [], onSuccess = false, payload = false, conversation_status_code = false) {
            ...
            conversation_status_code = conversation_status_code === false ? (last_message ? (admin ? 1 : bot_status_code) : -1) : conversation_status_code;
            if (this.conversation == false) {
                let last_conversation = activeUser().getLastConversation();
            ...
            if (message != '' || attachments.length || payload) {
                ...
                SBF.ajax({
                    ...
                    // Follow up
                    if (!admin && is_user && CHAT_SETTINGS['follow']['follow-active']) {
                        this.followUp();
                    }
                    ...
                    // Push notifications
                    if (!admin && is_user && !bot_unknow_notify && !agents_online && CHAT_SETTINGS['push-notifications']) {
                            ...
                            if ('queryResult' in response['response']) {
                                ...
                                for (var i = 0; i < messages.length; i++) {
                                    if ('payload' in messages[i]) {
                                        let payloads = ['human-takeover', 'redirect', 'woocommerce-update-cart', 'woocommerce-checkout'];
        ...
        initChat: function () {
            SBF.getActiveUser(true, () => {
                ...
                if (!active && (CHAT_SETTINGS['visitors-registration'] || CHAT_SETTINGS['welcome'] || CHAT_SETTINGS['subscribe'] || tickets) && (!tickets || !CHAT_SETTINGS['tickets-registration-required'])) {
                    this.addUserAndLogin(() => {
                        if (CHAT_SETTINGS['welcome-trigger'] != 'open' || this.chat_open) {
                            this.welcome();
                        }
                        if (CHAT_SETTINGS['subscribe']) {
                            this.subscribe();
                        }
                        this.finalizeInit();
                    });
                ...
                if (typeof SB_WP_WAITING_LIST !== 'undefined' && SB_WP_WAITING_LIST && SBF.storageTime('wating-list-' + SB_WP_PAGE_ID, 24)) {
                    setTimeout(function () {
                        SBApps.woocommerce.waitingList('request');
                    }, 1000);
                }
                setInterval(() => {
                    this.updateConversations();
                    ...
                    if (this.conversation != false) {
                        if (Array.isArray(response) && count > 0 && (!last_message || last_message.id != response[count - 1]['id'] || last_message.get('message') != response[count - 1]['message'])) {
                            ...
                            if (!admin && this.chat_open && SBF.isAgent(user_type) && user_type != 'bot') {
                                ...
                                if (CHAT_SETTINGS['follow']['follow-active']) {
                                    clearTimeout(timeout);
                                }
                                SBApps.dialogflow.active(false);
                            }
        ...
        // Subscribe message
        subscribe: function () {
            if (CHAT_SETTINGS['subscribe'] && !storage('subscribe') && activeUser() != false && SBF.null(activeUser().get('email'))) {
                setTimeout(() => {
                    SBF.ajax({
                        function: 'get-block-setting',
                        value: 'subscribe'
            ...
            if (next) {
                ...
                switch (name) {
                    ...
                    case 'buttons':
                        ...
                        break;
                    case 'button':
                        content = `<a href="${settings['link']}"${'target' in settings ? ' target="_blank"' : ''} class="sb-rich-btn sb-btn${settings['style'] == 'link' ? '-text' : ''}">${settings['name']}</a>`;
                        break;
                    case 'list':
    ...
    */
    ...
    var SBApps = {
        ...
        login: function () {
            if (this.is('wp')) {
                return CHAT_SETTINGS['wp-users-system'] == 'wp' ? (typeof SB_WP_ACTIVE_USER != 'undefined' ? SB_WP_ACTIVE_USER : false) : false
            }
            return false;
        },
        ...
        wordpress: {
            ...
            isLogin: function () {
                return CHAT_SETTINGS['wp'] && typeof SB_WP_ACTIVE_USER != 'undefined' && SB_WP_ACTIVE_USER !== false;
            },
        },
        ...
        woocommerce: {
            ...
            // Waiting list
            waitingList: function (action, product_id = false) {
                SBF.ajax({
                    ...
                    if (response) {
                        SBF.storageTime('wating-list-' + SB_WP_PAGE_ID);
                    }
                });
            }
        },
    ...
    $(document).ready(function () {
        ...
        if (main.length && typeof SB_AJAX_URL != 'undefined') {
            ...
            SBF.ajax({
                ...
                if (!admin && !CHAT_SETTINGS['chat-manual-init'] && (!CHAT_SETTINGS['disable-offline'] || agents_online) && (!CHAT_SETTINGS['disable-office-hours'] || CHAT_SETTINGS['office-hours']) && (!CHAT_SETTINGS['chat-login-init'] || (!SBApps.wordpress.is('wp') && SBF.cookie('sb-login')) || (SBApps.wordpress.is('wp') && SBApps.isLoggedWP()))) {
                    SBChat.initChat();
                }
